/**
 * Spawning the real entrypoint, shared by the tests that need a live server.
 *
...
export async function startServer(extraEnv = {}) {
  ...
  const child = spawn(process.execPath, [ENTRYPOINT], {
    env: {
      ...
      // server would stand on the LAN and the tailnet for the length of the
      // test (`docs/specs/server.md` -> Configuration).
  ...
  try {
    ...
    // server holding `port` for the rest of the run.
    child.kill();
    throw error;
  }
