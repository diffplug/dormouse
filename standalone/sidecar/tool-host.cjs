...
var tool_host_exports = {};
__export(tool_host_exports, {
  createToolHost: () => createToolHost
});
module.exports = __toCommonJS(tool_host_exports);

// ../lib/src/host/tool-registry.ts
var import_yaml = __toESM(require_dist(), 1);
var ToolFileError = class extends Error {
};
var SUBSTITUTIONS = ["$PROJECT_ROOT", "$CWD"];
var SUBSTITUTION_TOKEN = /\$[A-Za-z_][A-Za-z0-9_]*/g;
var KNOWN_PRESPAWN_FIELDS = /* @__PURE__ */ new Set(["prespawn_dedupe"]);
var KNOWN_ENTRY_FIELDS = /* @__PURE__ */ new Set(["run", "prespawn_dedupe"]);
function isRecord(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
function readDedupeTemplate(value, where) {
...
function validateSubstitutions(template, scope, where) {
  for (const element of template) {
    for (const token of element.match(SUBSTITUTION_TOKEN) ?? []) {
      if (!SUBSTITUTIONS.includes(token)) {
...
function parseToolFile(text, opts) {
  ...
  if (doc === null || doc === void 0) {
    return { scope, dir, tools: /* @__PURE__ */ new Map(), warnings: [] };
  }
  if (!isRecord(doc)) throw new ToolFileError(`${path}: expected a mapping at the top level`);
  const toolsNode = doc.tools;
  if (toolsNode === void 0) return { scope, dir, tools: /* @__PURE__ */ new Map(), warnings: [] };
  ...
  for (const [name, rawEntry] of Object.entries(toolsNode)) {
    ...
    let dedupeTemplate = null;
    if (rawEntry.prespawn_dedupe !== void 0 && rawEntry.prespawn_dedupe !== null) {
...
function resolveDedupeKey(entry, context) {
  if (!entry.dedupeTemplate) return null;
  return entry.dedupeTemplate.map(
    (element) => element.replace(SUBSTITUTION_TOKEN, (token) => {
...
var TOOL_FILE_NAME = "dormouse.yml";
var TRUST_FILE_NAME = "tool-trust.json";
function emptyTrust() {
  return { roots: {} };
}
var FileToolTrustStore = class {
  #dir;
  #path;
  #cache = null;
  constructor(stateDir) {
    this.#dir = stateDir;
    this.#path = (0, import_node_path.join)(stateDir, TRUST_FILE_NAME);
  }
  async get(root) {
    return (await this.#read()).roots[(0, import_node_path.resolve)(root)] ?? "unknown";
  }
  /** Record a decision a human made in Dormouse's chrome. */
  async set(root, decision) {
    const current = await this.#read();
    const next = { roots: { ...current.roots, [(0, import_node_path.resolve)(root)]: decision } };
  ...
  async #read() {
    ...
    try {
      const parsed = JSON.parse(await (0, import_promises.readFile)(this.#path, "utf-8"));
      const roots = parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed.roots : null;
      this.#cache = roots && typeof roots === "object" && !Array.isArray(roots) ? { roots } : emptyTrust();
    } catch {
  ...
  async #write(state) {
    await (0, import_promises.mkdir)(this.#dir, { recursive: true, mode: 448 });
    if (process.platform !== "win32") await (0, import_promises.chmod)(this.#dir, 448).catch(() => {
    });
    const tmp = `${this.#path}.${(0, import_node_crypto.randomUUID)()}.tmp`;
    await (0, import_promises.writeFile)(tmp, JSON.stringify(state), { mode: 384 });
    await (0, import_promises.rename)(tmp, this.#path);
  }
};
var MemoryToolTrustStore = class {
  #roots = /* @__PURE__ */ new Map();
  async get(root) {
    return this.#roots.get((0, import_node_path.resolve)(root)) ?? "unknown";
  }
  async set(root, decision) {
    this.#roots.set((0, import_node_path.resolve)(root), decision);
  }
};
async function findToolFile(startDir, readTextFile = (path) => (0, import_promises.readFile)(path, "utf-8")) {
  let dir = (0, import_node_path.resolve)(startDir);
  const { root } = (0, import_node_path.parse)(dir);
  for (; ; ) {
    const path = (0, import_node_path.join)(dir, TOOL_FILE_NAME);
    try {
...
async function lookupTool(name, cwd, trust, readTextFile) {
...
// ../lib/src/host/tool-host.ts
function createToolHost(options = {}) {
  const trust = options.stateDir ? new FileToolTrustStore(options.stateDir) : new MemoryToolTrustStore();
  return {
