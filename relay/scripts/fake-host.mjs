/**
 * Manual smoke tool: enroll a Host against a running selfhost Relay and run one
...
async function main() {
  ...
  const res = await fetch(`${relayUrl}${API_ROUTES.hostEnroll}`, {
    method: 'POST',
  ...
  const fakeHost = new FakeHost({
    ...
    // Minted locally and never sent to the Relay, exactly as a real Host does.
    noiseStaticKeyPair: await generateNoiseKeyPair(),
  });

  /** Mint a Relay setup token + a local invitation and print the code's URL. */
  const showCode = async () => {
    const minted = await fetch(`${relayUrl}${API_ROUTES.hostSetupToken}`, {
      method: 'POST',
