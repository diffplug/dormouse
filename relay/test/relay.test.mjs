...
import { connectClient, connectHost, freshApp, startRelay, wsConnect } from './helpers.mjs';
...
test('a host frame for a vanished client is dropped and the Relay keeps routing', async () => {
  const { app, server, close } = await relay();
  try {
    const { host, socket: hostWs } = await connectHost(app, server);
    const { socket: clientWs } = await connectClient(app, server);
