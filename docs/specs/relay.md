...
> Owns the selfhost Relay (`relay/`) and the shared Host-service runtime (`lib/src/host/remote/`). Read
...
## Where a Host may reach a Relay (self-host builds)
...
- `hosts.json` — `[{ hostId, hostToken, enrolledAt }]`; **no label** — the Relay keeps no name for a Host
...
`hostToken` — the host↔Relay bearer secret — in plaintext,
...
validated as `isE2eId` on both sides — the Relay reading `hosts.json`,
`isEnrollment` on the Host (rationale). A wrong shape reads as un-enrolled on the
Relay, and on the Host fails the exchange naming the field.
...
the same function the Host uses**, so Relay and Host cannot disagree on what a
valid assertion is.
...
`remote-lib-common/src/index.ts`, so Relay, Host and Pocket cannot drift.
...
| `POST /api/push/send`            | host token     | Fans out one sealed envelope per named delivery; `recipients` is required, and the Relay reads no notification text |
...
and `HOST_ENROLL_ATTEMPT_*` in `relay/src/app.ts`; test:
...
Relay's one third-party runtime dependency. Host and webview halves:
...
The Relay routes JSON envelopes between client sockets and host sockets
...
(Client→Relay, Relay→Host with `clientId` stamped, Host→Relay, Relay→Client
...
Source of truth: `relay/src/relay.ts` (`registerHost`), and `isE2eClientFrame` /
...
* **Enrollment** (Settings dialog, or the console hook, once): Relay URL + one
  ...
  `{ relayUrl, hostId, hostToken, origin, rpId }` (+ `requireUserVerification`
...
* **Setup codes**: `setupQr` — enrolled only — mints at `/api/host/setup-token`
  ...
  **while the invitation's *private* half stays in the Host process**, as does
  ...
  event; redemption at the Relay announces nothing. `host-fetch.ts` holds the
...
- **The password is passed through, never held**, cleared on success; `enroll`
  answers `{ hostId, relayUrl }`, so `hostToken` never re-enters the webview.
...
and WebSocket boundaries: the `FakeHost` in `relay/test/harness/fake-host.mjs`
...
`node relay/scripts/fake-host.mjs http://localhost:3000`
...
accepting any 200 on the port; and a Host reaching an installed Relay needs a
build whose baked relay allowlist admits the origin — a `*.ts.net` one means
`DORMOUSE_REMOTE_CONNECT_SRC` at build time ("Where a Host may reach a Relay" above).
