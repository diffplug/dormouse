...
- **FAIL IF** `.config/tend.yaml` does not set `merge: restricted`, or any `tend-*.yaml` passes another `merge:`; tend's `yolo` mode makes the bot a `Merge access` bypass actor.
