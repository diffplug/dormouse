...
- **Must continue from the last painted frame across Workspace activation**, not the incoming Wall's stale frame. Hidden Walls neither animate nor publish ring geometry. Tabs use Door geometry. Pinned by `carries the last visible ring across Walls instead of their stale pane positions` in `lib/src/components/wall/WorkspaceSelectionOverlay.test.tsx`.
...
**Must keep a selected Workspace tab a command-mode-only selection target.** Pane actions and terminal clipboard operations are inert there. Every passthrough entry selects a live pane. Pinned by `keeps removed Workspace commands inert on a selected tab` in `lib/src/components/WorkspaceWindow.test.tsx`.
