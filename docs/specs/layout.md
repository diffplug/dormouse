...
| **mount / unmount** | `mountElement` reparents the persistent DOM element into a container, `unmountElement` removes it. **The Registry entry survives**, and **neither fits the terminal** — the mount owns that ([Animations](#animations)). |
...
- **Must acquire GPU resources at mount, never at Session creation.** **Every release path — unmount/minimize, helper parking, Session disposal — disposes the addon**, then explicitly loses its context where capture and `WEBGL_lose_context` are available; **a throwing addon or extension must not abort the caller's teardown**. At mount it is the other way round: **must keep an activated renderer on WebGL when explicit loss is unavailable**, since a refused canvas probe or a missing extension is not a rendering failure and disposal still frees the GPU objects, leaving only context-slot reclamation to GC. (rationale) The xterm, grid, buffers, PTY, and other addons survive a minimize.
