...
- **Directional motion smear.** While travelling, each ring edge trails a soft band sized by its own motion. A line smears only by moving *across* itself — sliding along its own length leaves it unchanged — so **a horizontal edge is driven by its vertical speed and a vertical edge by its horizontal speed, and all four edges are independent.** The overlay finite-differences each edge position (`rect.top`, `rect.left + rect.width`, …) per frame, EMA-smooths the four speeds to shed rAF frame-timing jitter, and turns each into a band width (`strokeWidth + speed * smearGain`, clamped at `cfg.focusRing.smearMaxPx`). Alpha ramps from zero to `cfg.focusRing.smearPeakAlpha` over the same speed range, so an edge that is not moving contributes nothing rather than laying a band under the crisp ring. A settled or reduced-motion ring has null speeds and the smear layer is `display: none`, so snapshots stay deterministic. Tunables in `cfg.focusRing` (`smearGain` / `smearMaxPx` / `smearPeakAlpha` / `smearSmoothing`).
  - **Extent and intensity are deliberately independent.** `smearMaxPx` is how far the smear reaches, `smearPeakAlpha` how strongly it reads. Alpha is *not* divided by the widening factor: strict ink conservation ties peak alpha to the cap at `1/maxScale`, which makes the effect impossible to strengthen by widening — a wider band just spreads the same ink thinner and looks no more prominent. Reach for `smearPeakAlpha` to make the blur punchier and `smearMaxPx` to make it travel further.
