...
- **Snap gate.** `!cfg.layout.animate` (Chromatic) or `prefersReducedMotion()` → the ring settles instantly, mirroring the animator's 0-duration path (`lath-wall-engine.ts`). Only the unfocus-saturate fade keeps a CSS transition (`filter ${FOCUS_MOTION_MS}ms`), which `SelectionRing.tsx` sets inline — neither the snap gate nor reduced motion touches it. Chromatic is the single exception: `.storybook/preview.ts` zeroes every transition duration with an author `!important`, which outranks that inline declaration, so a snapshot shows the fade already finished. A missing fade in a Chromatic diff is therefore expected, not a regression.
