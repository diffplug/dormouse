...
- **Must separate highlighting from activation.** A click activates a highlighted tab; clicking `+` creates a Workspace and enters its pane after mount. Deactivation restores a Wall's chrome selection to its last live pane. External removal of the highlighted Workspace selects a live pane.
...
**Must bind only bare `1`–`9` to Workspace actions in command mode**, selecting by strip position and consuming out-of-range positions without switching. Workspace creation, close, rename, cycling, and strip navigation have no command-mode bindings. Rename inputs and confirmation dialogs retain their own controls. Pinned by `lib/src/components/wall/keyboard/handle-workspace-shortcuts.test.ts` and `lib/src/components/WorkspaceWindow.test.tsx`.
...
**Never navigate into or across the Workspace strip with arrow keys.** Up from a top-edge pane leaves its selection unchanged. Pinned by `lib/src/components/wall/keyboard/handle-pane-navigation.test.ts`.

**Must keep workspace tabs and `+` command-mode-only selection targets.** Pane actions and terminal clipboard operations are inert there. Every passthrough entry selects a live pane. Pinned by `keeps removed Workspace commands inert on a selected tab` in `lib/src/components/WorkspaceWindow.test.tsx`.
