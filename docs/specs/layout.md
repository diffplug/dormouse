...
12. **`dor ensure` is focus-neutral**: unlike `dor split`, `dor ensure` must never move focus off the caller — matched, restarted, or freshly created (`docs/specs/dor-cli.md`). Adding the pane `inactive` does **not** work: dockview renders and lays out a pane only once it becomes its group's active panel (a new group's `setActive` runs `doSetActivePanel`), so an `inactive` add — which skips both — runs the shell behind a blank tile until the first click activates it. Instead `createSplitSurface`'s `focusNeutral` option adds the pane **active** (so it renders), then hands the active group straight back to the caller via `restoreActivePanel.api.setActive()`; the new pane stays active *within its own group* and keeps rendering. `suppressActivationSelectRef` gates the `onDidActivePanelChange` listener (#11) across both activations so `selectedId`/mode never leave the caller. But adding the pane re-parents the caller's grid subtree in the DOM, which **blurs its xterm textarea** — and since `selectedId` never changed, `TerminalPanel`'s focus effect does not re-run to reclaim it, so the caller would silently stop receiving keystrokes until clicked. So after the add, `createSplitSurface` re-asserts the caller's focus with a deferred `focusSession` (rAF, to beat dockview's post-split focus handling, like the click-focus rAF in `enterTerminalMode`), gated on the caller still being the selected pane in passthrough — the same condition `TerminalPanel` uses for `isFocused`. `focusNeutral` also skips `selectPane`; the `--minimize` case passes `select: false` to `minimizePane` so it creates the door without stealing selection or switching to command mode. If a freshly created shell never reports OSC 633 integration, ensure tears the throwaway pane down with `killPaneImmediately`; because the focus-neutral create never selected it, `orchestrateKill`'s `wasSelected` gate (see the kill "Selection tail" above) leaves the caller's selection intact.
