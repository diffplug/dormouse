...
- If scrolled, show `← N more` on the left and/or `N more →` on the right. Overflow counts are assumed single-digit (the hidden measurement buttons are `9 more` and `9 more TODO`).
- **An arrow hiding a ringing or TODO Door must say so**, since that Door is otherwise gone from the baseboard: it wears the Door ground, a static `door` alarm inset while one rings, and a `TODO` mark while one has TODO, and its accessible name counts them (`3 more, 1 ringing, 1 TODO`). **Must reserve the measured TODO arrow width** wherever the Doors an arrow would hide include a TODO. Pinned by `Baseboard overflow alerts` in `lib/src/components/Baseboard.test.tsx`.
