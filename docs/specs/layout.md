...
2. **Restore** (app restart, cold start): the Wall's `seed` hydrates from the restored Lath layout, else falls to (3); `restoreTerminal()` per pane with its saved cwd and title, plus the single-use agent resume invocation the host captured (`docs/specs/transport.md` → "Consuming it") and, on a host that cannot seed itself (standalone), the pane's persisted TODO through `PlatformAdapter.alertSeed`, sent after the spawn. Browser surfaces are rebuilt from their persisted params instead.
