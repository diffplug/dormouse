...
- **Directional motion smear.** While travelling, each ring edge trails a soft band sized by its own motion. A line smears only by moving *across* itself — sliding along its own length leaves it unchanged — so **a horizontal edge is driven by its vertical speed and a vertical edge by its horizontal speed, and all four edges are independent.** The overlay finite-differences each edge position (`rect.top`, `rect.left + rect.width`, …) per frame, EMA-smooths the four speeds to shed rAF frame-timing jitter, and turns each into a band width (`strokeWidth + speed * smearGain`, clamped at `cfg.focusRing.smearMaxPx`). Band alpha is divided by the widening factor so the smear conserves ink the way a real one does, and fades in from zero, so an edge that is not moving contributes nothing rather than laying a solid band under the ring. A settled or reduced-motion ring has null speeds and the smear layer is `display: none`, so snapshots stay deterministic. Tunables in `cfg.focusRing` (`smearGain` / `smearMaxPx` / `smearSmoothing`).
