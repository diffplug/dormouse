...
- **Must make a tab's TODO pill (`docs/specs/alert.md` → Workspace union) its own button, beside the tab's and never inside it.** Its click activates the Workspace in command mode and selects the next member with a TODO after the current selection — panes in tree pre-order, then Doors in baseboard order — wrapping; a Door is selected, never reattached. **With no TODO left it only activates in command mode, never renaming.** Pinned by `activates a Workspace from its tab TODO pill onto its TODO pane` in `lib/src/components/WorkspaceWindow.test.tsx` and `cycles through TODO members in command mode, Doors included, with no alert verb` in `lib/src/components/Wall.test.tsx`.
- **Must draw the tab's TODO pill borderless**, as text on a rounded hit area inside the tab's height, with a pointer, a hover wash, a stronger pressed wash, and a current-coloured focus-visible outline. Its tooltip names the Surface a click selects as its header or Door shows it, re-read on hover, focus, and each click; that Surface plays the landing spotlight (`docs/specs/alert.md` → Pane Header). Pinned by `draws the pill borderless, with hover, press, pointer, and focus-visible states` and `names the Surface the click will select in the pill tooltip` in `lib/src/components/WorkspaceStrip.test.tsx`.
