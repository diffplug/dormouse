...
- **Wash + label at `z-index: 19`** — below the header, so the wash never tints the header band, where `--color-alarm-vs-terminal` (picked against the *terminal body*) carries no contrast guarantee; above terminal content; below the `z-index: 20` pane-corner mouse-override banner, which therefore stays untinted. Both states wash, `SPEAKING` at 20% and `SPOKEN` at half that: `SPOKEN` persists until the ring is attended, which is unbounded, so it keeps a light haze that still reads as an unhandled alarm without degrading terminal-text contrast the way a full-strength tint would for that whole window. A Door has no equivalent (Doors, below): a haze across a 24px chip reads as a color shift rather than an alarm, so there `SPOKEN` rides on the inset alone. The label sits `PANE_HEADER_HEIGHT_PX + 4` from the Pane top, centered, in both states.
