...
- **Retired** by taking it, dismissing it, the user's first input into the pane (keyboard input through xterm, direct clipboard paste, or file-drop path insertion), or session dispose. Replay-shaped terminal reports do not count. It does not survive into the next save — the offer lives only in the runtime store, and the next restore re-seeds from the snapshot.
