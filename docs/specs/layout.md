...
- **Must dispose the addon on unmount/minimize, helper parking, and Session disposal**, then explicitly lose its context when captured and supported. Addon or extension failures must not abort teardown. Minimize preserves the xterm, grid, buffers, PTY, and other addons.
