...
12. **`dor ensure` is focus-neutral**: unlike `dor split`, `dor ensure` must never move focus off the caller — matched, restarted, or freshly created (`docs/specs/dor-cli.md`). `createSplitSurface`'s `focusNeutral` option adds the new pane with `inactive: true` (suppressing the auto-activation in #11 and its selecting `onDidActivePanelChange`) **and `renderer: 'always'`** — dockview only mounts an inactive panel's content when its renderer is `'always'` (`doAddPanel` skips `openPanel` for a `skipSetActive` add unless the renderer forces a render), so `inactive` alone spawns the shell but shows a blank pane. It also skips `selectPane`; the `--minimize` case passes `select: false` to `minimizePane` so it creates the door without stealing selection or switching to command mode. If a freshly created shell never reports OSC 633 integration, ensure removes the throwaway pane with a plain `disposeSession` + `api.removePanel` rather than `killPaneImmediately`, whose kill animation would reselect `panels[0]`.
