...
Zoom is not in the tree. It is presentation state (`zoomedId` in the wall store): the zoomed leaf animates into an elevated wall-sized rect inset by 6px, while the tree and all other rects stay unchanged beneath. The exposed perimeter makes the stacking relationship visible. This replaces `maximizeGroup` without moving or reordering the leaf DOM.
...
- Zoom retargets only the chosen leaf to the wall rect inset by 6px and elevates it above tiled/dying panes and sashes. The animator expands it from its tiled rect; unzoom keeps it elevated while it shrinks back, lowering it only after the return frame settles. The surrounding 6px band reveals the unchanged tiled layout beneath.
