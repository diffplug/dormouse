...
Source of truth: `ParkedLeaf` / `parked` / `parkLeaf` / `unparkLeaf` / `parkedIds` /
`leafMetaIn` / `MAX_PARKED_SURFACES` in `lib/src/components/wall/lath-wall-store.ts`;
...
- **`leafMeta` and `parked` are disjoint**: a leaf is laid out or parked, never both.
  That is what keeps parked meta out of the persisted layout with no filter, and it
  makes `leafMetaIn` / `getMeta` a plain either-or lookup.
- **A parked Surface keeps running**, so its meta keeps changing (an iframe reporting a
  ...
  leaves, and the store — not the Door record frozen at minimize time — is the
  authority for every operation on one: reattach (click or drag-out), dor param lookup,
  kill/session teardown, `buildDorSurfaces`, and session save all read through the
  live-first `lath.doorMeta(door)` seam. The Door *chip's* own label stays as minimized
  (it is Wall state, and it read a dead leaf's meta before parking existed too).
...
- **Bounded.** Each parked leaf is a live document still running scripts, timers, and
  ...
  in the same commit, so no caller can forget. An evicted Surface stays a perfectly
  good Door — it just reattaches by reloading. The cap is generous for the minimize
...
- Binding (jsdom): **node identity is preserved** across every op (the no-re-parent guarantee) and DOM order stays fixed while layout order changes; parked leaves keep the same node and children across park/restore, hold their last rect while hidden (asserted under StrictMode, where every ref detaches and re-attaches), sort in place, receive `parked` as a pane prop, and unmount only on `unparkLeaf`; imperative frame application between commits (fake rAF + fixed-duration engine), mid-tween React re-renders not snapping styles, dying pointer-events; sash drag preview/commit/cancel and the snap-on-commit; the pane-drag gesture (threshold entry, button bail, preview overlay, wheel depth cycling, baseboard-zone minimize, Escape cancel, external door-drag mode); inset zoom expansion/return with its elevated layer lifetime; the pane props contract via `componentsOverride`; store mutator/rejection/notify semantics; the read-boundary layout resolution (`session-restore.test.ts` / `reconnect.test.ts`); engine hydration from a persisted Lath layout and from fresh pane ids; a `<Wall>` smoke (split, kill, Lath-layout save capture).
