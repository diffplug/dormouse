...
- **FAIL IF** either package lacks a verified npm SLSA provenance attestation binding its installed version to `diffplug/pgstencil` `.github/workflows/release.yml` on `refs/heads/main` and the same commit as `dist/provenance.json`. Verify the signed subject against the installed registry package before trusting the statement.
