...
Pairing UI events are the opposite: unaddressed and broadcast to every window's webviews, because the approval modal must appear wherever the user happens to be looking. A window that is neither contending nor connected refuses a command with an error rather than dropping it, so the console hook fails fast instead of hanging for its whole timeout.
