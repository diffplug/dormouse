...
Mostly these are the tests that need real I/O, since the pure halves already live in `lib`. Six suites under `vscode-ext/test/`, plus `helpers.ts` and `vscode-stub.ts`:
