...
**The store.** The Host's enrollment (`{ serverUrl, hostId, hostToken, origin, rpId, label }` plus this machine's Noise static) and its ACL are split by sensitivity: `hostToken` and the static's private half are credentials that grant the `/ws/host` socket and this Host's identity, so the enrollment goes to `SecretStorage` (OS keychain), while the ACL — public keys plus each Client's push `deliveryId` — goes to `globalState`. Both are global rather than workspace-scoped, because a Host identity belongs to the machine and not to a folder.
...
`webview-boot.smoketest.ts` is separate: `pnpm --filter dormouse test:smoke` runs the shipped webview under Chromium against a prebuilt `media/`, and CI runs it in its own job. It must stub `acquireVsCodeApi` so the VS Code-only lazy import executes; the smoke assertions pin that path.
