...
- **Each router is one engagement viewer** of it (`docs/specs/alert.md` → Engagement), set by `alert:engagement`. **Must remove the viewer on router disposal and on a `dormouse:init` re-init**, which also cancels that router's parked awaits: recreated content is a new realm (`endRealm` in `vscode-ext/src/message-router.ts`). Pinned by `engagement viewers` in `vscode-ext/test/message-router.test.ts`.
...
- **Must reserve a closing router's PTYs until its deferred kills finish**, so another router cannot claim them during archive/CWD work, then **remove the alert entry of each PTY it kills**, its webview being gone. Pinned by `vscode-ext/test/message-router.test.ts`.
- **A remote Client's write acknowledges with input before it is written**, whichever Burrow tier it came through (`docs/specs/alert.md` → Engagement; `writeClientInput` in `vscode-ext/src/message-router.ts`).
...
The extension host is the platform host of `docs/specs/transport.md` → "PTY
lifecycle": `pty-manager.ts` forks the pty-host child, the bottom-panel
`WebviewView` and each editor-tab `WebviewPanel` are the webviews, and each
router owns its own PTY ids. Consequences:
