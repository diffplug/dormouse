...
| **The self-host HTTPS origin may be public; its plaintext backend may not.** Setup credentials are generated 256-bit values, Host enrollment is globally admission-limited, cross-origin browsers receive no grant, and terminal access still needs local Host approval. | [The setup password](./security-remote.md#the-setup-password), [Network posture](./security-remote.md#network-posture-self-hosted) | `server/test/config.test.mjs`, `server/test/app.test.mjs`, `server/test/token-bucket.test.mjs`, `server/test/cors.test.mjs`, `scripts/deploy-lint.mjs` |
