...
**Why the origin carries a CSP at all.** Pocket holds a per-Host Client static and the worker that opens sealed pushes, and `SECURITY.md` -> Accepted limitations already names active XSS here as a risk it cannot rule out. Both shipped webview hosts already have a policy, leaving Pocket the one origin without one.
