...
- **Every fragment's verdict line and its `UNVERIFIABLE`, `FAIL:`, `BLOCKER` and `WARNING` lines are lifted into the head ahead of the report**, matched at line start after an optional heading or bullet marker so the fragments' `FAIL IF` vocabulary is not read as a finding. Truncation keeps the head, so otherwise the markers the notes point at are what gets cut (rationale).
