...
- **Must show Settings as one continuous page with a left-side General, Activity, Notifications, and Notepad table of contents**, omitting General when neither theme nor shell is editable and Notepad without an archive port. General holds theme and shell; Activity holds watching and inactivity; Notifications holds application defaults, Workspace overrides, and Remote control; Notepad holds the archive entry. Keep Remote control below push settings.
- **Must smoothly scroll on contents click, mouse hover, or keyboard activation**, without hiding sections. Highlight a hovered contents entry or settings section ahead of scroll position; hovering settings never scrolls. Otherwise follow the section at the scroll area's top, selecting the last at the bottom. Keep the contents visible while scrolling.
- **Must search available settings’ labels, descriptions, rendered options, and live command/device names**, case-insensitively, requiring every whitespace-separated search term within a group or its topic name. Show matching groups and contents entries, or an empty state; clearing search restores all sections. Search changes smoothly scroll to the top. Preserve mounted controls through filtering/navigation.
- **Must close Settings on a backdrop click, the close button, or Escape**, with Escape closing an open picker first. Initially focus search; support Up/Down/Home/End in the contents and exclude hidden controls from Tab traversal. Pinned by `lib/src/components/SettingsDialog.test.tsx` and `lib/src/stories/SettingsDialog.stories.tsx`.
