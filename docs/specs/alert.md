...
Spoken-alarm delivery is much louder than the bell. While the engine is actually speaking, a pointer-transparent treatment spans the whole terminal Pane: a wash, an animated high-contrast inset, and a `SPEAKING` label. After the utterance settles the animation stops; a static inset, a `SPOKEN` label, and a half-strength wash remain until the ring resolves — `SPOKEN` is an unbounded window, so **the haze stays light enough to read terminal text through**. **`prefers-reduced-motion` keeps the strong static treatment and suppresses only the pulse**, as does `cfg.alert.ringingPaused` (rationale). Layering, placement, and sizing belong to `docs/specs/layout.md`.
