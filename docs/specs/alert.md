...
- The settings dialog re-reads the device list when it opens (`refreshPushDevicesNow`). A phone can enable alerts long after this machine booted, so a list fetched only at Host start would name the wrong devices — or none — for the rest of the session. A refresh whose request is still in flight when the Host stops (or is replaced by re-enrollment) discards its result rather than overwriting `no-host` with a stale list.
