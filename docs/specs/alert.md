...
Spoken-alarm delivery is deliberately much louder than the bell. While the engine is actually speaking, a pointer-transparent treatment spans the whole terminal Pane with an animated high-contrast inset and an explicit `SPEAKING` label. After the utterance settles, the animation stops but a static high-contrast inset and `SPOKEN` label remain until the ring is resolved. `prefers-reduced-motion` keeps the strong static `SPEAKING` treatment and suppresses only the pulse, as does `cfg.alert.ringingPaused` (the Chromatic freeze that pins the bell — an infinite opacity cycle would otherwise snapshot at an arbitrary phase). Layering, placement, and sizing belong to `docs/specs/layout.md`; source of truth: `lib/src/components/wall/AlertSpeechIndicator.tsx`.
