...
1. **Update dependency snapshots** — run `node website/scripts/generate-deps.js` and review the diffs in `website/src/data/dependencies.json` and `website/src/data/cargo-dependencies.json`. Commit if changed.
