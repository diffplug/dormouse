...
|----------|-------------------------------------------------|
| macOS app bundle | `standalone-mac-aarch64/src-tauri/target/aarch64-apple-darwin/release/bundle/macos/MouseTerm.app` |
| Windows app executable | `standalone-win-x64/src-tauri/target/x86_64-pc-windows-msvc/release/mouseterm.exe` |
| Windows installer | `standalone-win-x64/src-tauri/target/x86_64-pc-windows-msvc/release/bundle/nsis/MouseTerm_X.Y.Z_x64-setup.exe` |
...
| Linux AppImage | `standalone-linux-x64/src-tauri/target/x86_64-unknown-linux-gnu/release/bundle/appimage/MouseTerm_X.Y.Z_amd64.AppImage` |

Release upload likewise uses only the three stable output filenames (`MouseTerm-macos-aarch64.tar.gz`, `MouseTerm-windows-x64-setup.exe`, `MouseTerm-linux-x86_64.AppImage`) and fails if `release-signed/release-assets` contains any other files.
