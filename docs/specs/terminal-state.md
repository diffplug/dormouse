...
- **Boundary mode, plus a trailing-boundary trim.** Stripping runs in **boundary mode** for the same reason resume detection does: deleting a redraw's cursor move welds text that was never adjacent on screen, and this reads the result as a *line*. `building...\x1b[1;1HC:\Users\me>` would otherwise read as one line starting with `building`, so the anchored cmd.exe/PowerShell/arrow/bare-terminator shapes stop matching and the pane never looks idle. But a boundary is not a real line break, and the difference decides the safe direction: a **genuine** trailing newline means nothing is painted on the current line yet — no prompt — and must keep reading as `null`, because a false positive flips a running command back to idle; a **boundary** at the tail means only that a control sequence closed the line, which is what a prompt that clears to end-of-line after painting itself (`C:\Users\me>\x1b[K`) does, and treating that as an empty last line would hide every such prompt. Stripping the same text *without* boundaries leaves exactly the real breaks, which is how the two are told apart. Both directions are pinned by tests in `terminal-state-store.test.ts`.
