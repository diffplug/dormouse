...
- **Must key fallback state, including learned prompt shapes, by the stable Session id** (`docs/specs/layout.md` → Session lifecycle and terminal registry). Pinned by `keeps Session state across a Lath swap` in `lib/src/lib/terminal-state-store.test.ts`.
...
**Must apply asynchronous process-CWD results to their originating Session and drop results for a disposed Session.** Pinned by `keeps CWD with its Session across a Lath swap` and `does not resurrect a disposed pane when a late process CWD arrives` in `lib/src/lib/terminal-state-store.test.ts`.
