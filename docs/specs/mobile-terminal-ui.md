...
* The mobile session header is a flush bar — **never** the desktop title corner
  ...
  the bar and on its session-list row. **A touch acknowledges the active
  Session in `MobileTerminalUi`'s capture phase**, before any touch mode
  consumes it; both consumers route its input bar and gesture keys through
  `writeUserInput` (`docs/specs/alert.md` -> Engagement).
