...
**The managed-voice token is a bearer credential at rest** — `<state dir>/managed-voice.json` beside the Burrow's enrollment, written by `writeJsonAtomic` (`0700`/`0600`; on Windows the owner-only DACL `burrow_state_dir` applies before the sidecar spawns), with the voice id (rationale). Standalone only; **never returned to a webview** (`docs/specs/alert.md` → "Managed voice"). **The token must go only to `https://hosted.dormouse.sh`, or to the dev override `DORMOUSE_HOSTED_ORIGIN` when it parses as a bare `http:` origin on `127.0.0.1`, `[::1]`, or `localhost`**; any other value is ignored, and a release build's Rust removes the variable before the sidecar spawns. Pinned by `dev Hosted origin override` in `lib/src/host/managed-voice-host.test.ts`. The request is outbound, not a listener, so "Loopback Listeners" does not apply.
