...
- `SiteHeader` at top, `themeAware` so `--vscode-*` variables drive its chrome. It carries no controls: theme selection moved into the Wall's Settings dialog (`docs/specs/theme.md`), so the page restores its own theme with `useRestoredTheme(POCKET_THEME_ID)` (`lib/src/lib/themes/use-restored-theme.ts`) rather than relying on the picker mounting, and passes the same fallback to `Wall` for the Settings picker.
