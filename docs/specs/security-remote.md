...
- **FAIL IF** a switched end waits on its peer without a deadline, or a channel a Noise stream cannot ride is adopted. `DirectPeer` in `lib/src/remote/direct/direct-peer.ts` must refuse a channel that is unordered, partially reliable, or not `DIRECT_CHANNEL_LABEL`, and one whose association reports a per-message limit below `NOISE_MAX_MESSAGE_LENGTH`, both before it reports the open — each abandons the attempt while the relay still carries the session. `DirectEndpoint` must arm `DIRECT_HANDOFF_TIMEOUT_MS` on its own switch, since from there it sends only on the channel. Pinned by `lib/src/remote/direct/direct-peer.test.ts` and `lib/src/remote/direct/direct-endpoint.test.ts`.
