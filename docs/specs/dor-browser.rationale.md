...
**Why an unpark catches up only once its socket opens.** The unpark entered `live` at once, so sync-to-pane and a pending navigation ran before the parked port was proven. With the daemon gone while hidden (`dor ab close`, a crash) and the pane resized meanwhile, as restoring a Door does, `set viewport` started a fresh `about:blank` daemon through the CLI, and the failed stream's attach then streamed it instead of ending (review of #775, 2026-09). The host now refuses that command outright; waiting still keeps the resize and the page owed, rather than refused and lost, until the unpark finds its browser or ends.
