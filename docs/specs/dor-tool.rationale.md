...
**Why autobind waits a tick.** Ports appear one at a time during boot. The standalone harness binds its dev bridge (1422) before vite (1420), so a scan landing between the two sees only the bridge — and because a framed leaf is never re-scanned, framing on first sighting would frame the bridge permanently. Waiting for one unchanged tick costs ~1.5s, catches every boot-time case, and never has to retract a browser someone is already using. Scanning forever after serving was the alternative; it pays a shell-out per tool per tick for the life of the tool, to catch a case that essentially only happens at startup.
