...
Structure only: panes (id, cwd, title, `untouched`, `surfaceType`, TODO/alert blob, plus a tool's command and stable metadata), doors and their Lath restore tokens, the Lath layout, and the Workspace's `dor` surface refs. **Scrollback is never persisted by any writer**, and neither is the recovery command (see above).
