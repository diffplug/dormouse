...
5. If the saved session covers those live PTYs, the frontend uses the saved Lath layout when its leaf set matches (migrating a legacy dockview blob if that is all a pre-Lath save carries) and reattaches saved minimized doors; minimized PTYs are registered but remain doors instead of visible panes.
```

For cold restore (no live PTYs), the webview falls back to saved session state: spawns new PTYs in saved CWDs using the currently selected Dormouse shell, injects saved scrollback (with trailing newline to avoid the zsh `%` artifact), and restores the saved Lath layout (migrating a legacy dockview blob one-way for pre-Lath saves). The entry module (`reconnect.ts`) uses a 500ms timeout when waiting for the PTY list.
...
Source of truth: `lib/src/lib/session-types.ts` defines the persisted-session interfaces (`PersistedSession` v3, `PersistedPane` — now carrying `surfaceType` — `PersistedAlertState`, `PersistedDoor`, and the container types `PersistedWorkspace` / `PersistedWindow`) and their migrations.

**The layout field.** A `PersistedSession` records the layout as `lathLayout` — the native Lath tree (`docs/specs/tiling-engine.md` → "Persistence and migration"). The legacy dockview `layout` key is retained as an **optional, read-only** field: it appears only in blobs written before Lath and is migrated one-way to a Lath tree on restore; new saves never write it. The v3 guard accepts a blob carrying either field. `PersistedDoor`'s pre-Lath positioning fields (`neighborId` / `direction` / `remainingPaneIds` / `layoutAtMinimize` / `layoutAtMinimizeSignature`) are likewise optional legacy-read-only; a new door carries a Lath restore `token` instead.

**Surface kinds in the snapshot.** Each `PersistedPane` records a `surfaceType` (`docs/specs/glossary.md`): `'terminal'` (the default, omitted from the row to keep terminal snapshots byte-identical) or `'browser'`. This is the discriminator that routes restore/resume. `restoreSession` skips terminal restoration for a browser pane, so it no longer mints a stray PTY + xterm for each browser pane id (`session-restore.ts`); the resume plan keeps browser panes (and minimized browser doors) even though they have no live PTY, so the saved layout's pane set still matches and is not discarded (`reconnect.ts` gates the session's single Lath channel — a legacy `layout` blob is migrated at the read boundary before gating — on its leaf set). A browser pane rebuilds from the persisted layout (visible) or `PersistedDoor.params` (minimized); its render params (`renderMode`, `url`, agent-browser `session`) live there, not in `PersistedPane` — `surfaceType` alone is enough to route restore. A pane lacking `surfaceType` reads as `'terminal'`, so older snapshots migrate transparently.
