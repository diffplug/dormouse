...
- **`access-control-allow-origin` names the Vite origin exactly, never `*`**, on every response including the SSE stream. Under `*` the `read_clipboard_text` and `read_clipboard_file_paths` invokes were readable cross-origin.
