...
**Why the per-sequence byte caps are above upstream.** The wire caps go the other way: one 33,554,432-byte (32 MiB) number for SIXEL, IIP, and Kitty replaces upstream's three different values (25,000,000 / 20,000,000 / 20,000,000), so there is a single encoded-size bound to reason about and a 4K PNG is not rejected at the sequence boundary before the pixel ceiling can judge it. `sixelPaletteLimit` is likewise raised from 256 to the SIXEL-spec maximum of 4,096. Decoded memory is bounded by the pixel and storage ceilings above regardless of encoded size.
