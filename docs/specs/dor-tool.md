...
4. **Must require `trust-recorded` before re-resolving the named entry**, retaining the pending pane and its error until the next attempt after a rejected grant or failed re-resolution (blank reasons use a fallback), then stage the resolved command, renderer, port strategy, and key before exposing its terminal. A Surface closed during host calls must not start later or show a stale error.
