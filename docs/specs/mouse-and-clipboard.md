...
**Must defang every bracketed payload:** replace each `\e` with visible U+241B before wrapping, or `\e[201~` can close the boundary and let later newlines submit. This covers file-path pastes (§8.6 tiers 1 and 3), which share the writer. **Never filter the unbracketed branch:** without a paste boundary, filtering only corrupts deliberate escape sequences. Both branches are pinned by `lib/src/lib/clipboard.test.ts`.
