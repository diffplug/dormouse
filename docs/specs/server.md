...
`docs/specs/security-application.md` -> "Loopback Listeners" requires; `scripts/loopback-lint.mjs` does
...
- **Endpoint egress is public HTTPS only** — the one path where the Server makes
  ...
  second unchecked resolution. Range list: `docs/specs/security-application.md` -> "What crosses the boundary".
...
- **The offer's token never enters a webview.** `status` carries only the origin;
  ...
  reuse a spent offer (`docs/specs/security-application.md` -> "Credentials at rest").
...
there, audited by the `FAIL IF` lines in `docs/specs/security-application.md` and checked textually by
