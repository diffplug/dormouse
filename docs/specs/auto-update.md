...
**The updater owns no quit interception** — install runs only when `hasPendingUpdate()` is true, after the quit orchestrator's teardown and save/drain steps (`docs/specs/standalone.md` §Quit flow) (rationale). **It runs in whichever window the quit walk tears down last** — `main` while it is open, else the most recently focused one — so every window holds `updater:*`: a capability split pinning the install to `main` would leave a session whose `main` was closed silently unable to update. Every other window has handed on by then, so nothing it could still be writing outlives the install.

**Only `main` runs the periodic check**, so a session whose `main` was closed has none until it relaunches, and **closing `main` throws away an approved download** — it lives in that webview's memory. Its close confirmation says so, and is shown for that reason alone even with nothing running (`docs/specs/standalone.md` → "Per-window close"). `installPendingUpdate()` writes the success marker *before* `install()` (§localStorage), and on Windows first awaits bounded sidecar teardown (§Sidecar teardown on Windows). **It never closes the window itself** — exiting the process is `quit_proceed`'s job, after this returns.
...
| [`standalone/src-tauri/capabilities/default.json`](../../standalone/src-tauri/capabilities/default.json) | Shell, window, updater and app-version permissions, for `main` and every `ws-*` window |
