// Diagnostic databases are independent of Pocket's authorization database: each
// run opens its own temporary database under PREFIX and deletes it afterwards,
// reporting both stage failures and cleanup failures, and every primitive check
// generates its own key material rather than reusing another check's. Nothing
// here reads pairing data, asks for a passkey or a media permission, or uploads
// anything (`docs/specs/pocket-app.md` -> "The capability harness").
...
const LIMIT = 8000;
export const message = error => `${error?.name || 'Error'}: ${error?.message || 'failed'}`;
export const assert = (ok, reason) => { if (!ok) throw new Error(reason); };
export const equal = (a, b, reason = 'Key operation produced a different result') => {
  const x = new Uint8Array(a), y = new Uint8Array(b);
  assert(x.length === y.length && x.every((v, i) => v === y[i]), reason);
};

/**
 * Write inside `tx` and wait for it to commit, aborting on a throw so a failed
 * check leaves no half-open transaction behind. `write` runs inside the try, so
 * a clone that the `put` itself refuses takes the same path as a failed commit.
 */
export async function commit(tx, write, reason = 'Transaction aborted') {
  const done = new Promise((resolve, reject) => {
    tx.oncomplete = resolve;
    tx.onabort = tx.onerror = () => reject(tx.error || new Error(reason));
  });
  try {
    write();
...
export async function runCapabilities(onResult = () => {}) {
  ...
  await check('encrypted-x25519', 'Production format: AES-protected X25519 with bound context, reopen and use', async phase => {
    phase('generate');
    const pair = await bounded(generatePocketKeyPair('encrypted', 'diagnostic'));
    ...
    const saved = await roundTrip({
      id: 'test', envelope: storePocketPrivateKey(pair.privateKey), publicKeyRaw,
    }, true, phase, report.cleanupErrors);
    phase('restore-production-key');
    const restored = await bounded(loadPocketPrivateKey(saved.envelope, 'diagnostic', saved.publicKeyRaw));
