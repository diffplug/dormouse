...
function text(value, fallback) {
  ...
  const cleaned = value
    ...
    // embedding/override marks, bidi isolates, and the BOM.
    .replace(/[\u061c\u200b-\u200f\u202a-\u202e\u2060-\u2064\u2066-\u2069\ufeff]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
  // Capped in code points, not UTF-16 units: a `.slice` landing mid-surrogate
  // would ship a lone half that renders as U+FFFD.
  const bounded = Array.from(cleaned).slice(0, TEXT_LIMIT).join('').trim();
...
function readPayload(event) {
  ...
  try {
    ...
    // Guarded again: a payload that fails both reads must still notify, or the
    // browser charges the userVisibleOnly penalty this worker exists to avoid.
    try {
