...
function text(value, fallback) {
  ...
  const cleaned = value.replace(/\s+/g, ' ').trim().slice(0, TEXT_LIMIT).trim();
  return cleaned || fallback;
}

function readPayload(event) {
  if (!event.data) return { title: FALLBACK_TITLE, body: FALLBACK_BODY, tag: undefined };
  let parsed;
  try {
    parsed = event.data.json();
  } catch {
    // Not JSON — surface whatever text arrived rather than dropping the alarm.
    return { title: FALLBACK_TITLE, body: text(event.data.text(), FALLBACK_BODY), tag: undefined };
  }
  if (!parsed || typeof parsed !== 'object') {
    return { title: FALLBACK_TITLE, body: FALLBACK_BODY, tag: undefined };
  }
  return {
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(
    self.registration.showNotification(title, {
      body,
      tag,
      // Replace silently: a re-ring of the same Pane should update the existing
      // notification, not buzz the phone a second time for the same alarm.
      renotify: false,
      icon: '/icon-192.png',
      badge: '/icon-192.png',
    }),
