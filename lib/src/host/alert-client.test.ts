...
it('sends each alert verb as one command naming its op', () => {
  ...
  methods.alertPublishSessions({ p: { label: 'pnpm build', overrides: { speakEnabled: true } } });
  ...
  expect(commands).toEqual([
    ...
    { op: 'sessions', sessions: { p: { label: 'pnpm build', overrides: { speakEnabled: true } } } },
...
describe('events', () => {
  ...
  it('hands the host\'s state and store snapshots to the renderer, and claims nothing else', () => {
    ...
    const stopSpeaking = methods.onAlertSpeak((speak) => void seen.push(['speak', speak]));
    ...
    // Only a speak's own fields reach the handler; one missing either is dropped.
    alerts.onEvent('alert:speak', { id: 'p', episodeId: 'e-1', forWindow: 'main' });
    alerts.onEvent('alert:speak', { id: 'p' });
    stopSpeaking();
    alerts.onEvent('alert:speak', { id: 'p', episodeId: 'e-2' });
    expect(alerts.onEvent('pty:data', { id: 'p', data: 'x' })).toBe(false);

    expect(seen).toEqual([
      ['state', detail], ['watched', ['make']], ['settings', DEFAULT_ALERT_SETTINGS],
      ['speak', { id: 'p', episodeId: 'e-1' }],
    ]);
  });
});
...
describe('awaits', () => {
  ...
  it('takes the host\'s answer through the real host, a claim included', async () => {
    const host = createAlertHost({ push: () => {} });
