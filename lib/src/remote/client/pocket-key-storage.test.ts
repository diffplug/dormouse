...
import { probePocketKeyStorage } from './pocket-db';
...
it('reports a rejected key clone before pairing, and cleans up', async () => {
  ...
  await expect(probePocketKeyStorage()).rejects.toThrow('Diagnostic: write-record / DataError.');
...
it('rejects a successful write whose private key does not survive readback', async () => {
  ...
  await expect(probePocketKeyStorage()).rejects.toThrow('Diagnostic: validate-record / Error.');
});
...
it('rejects a readback key that derives the wrong secret', async () => {
  ...
  await expect(probePocketKeyStorage()).rejects.toThrow('Diagnostic: compare-key-agreement / Error.');
});
...
it('identifies read failures without echoing browser error contents', async () => {
  ...
  const error = await probePocketKeyStorage().catch(error => error as Error);
...
it('distinguishes generation failure from storage failure', async () => {
  ...
  await expect(probePocketKeyStorage()).rejects.toThrow('Diagnostic: generate-key / NotSupportedError.');
});
...
it('allows a fresh retry after a storage failure', async () => {
  ...
  await expect(probePocketKeyStorage()).rejects.toThrow('Diagnostic: write-record / DataError.');
  await expect(probePocketKeyStorage()).resolves.toBeUndefined();
});
