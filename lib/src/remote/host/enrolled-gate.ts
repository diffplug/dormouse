/**
 * Arm something only while there is a Host to serve.
 *
 * Answering the Host is free — a webview replies to an ask and goes back to
 * sleep — but *volunteering* is not: announcing that the directory may have
 * changed costs a crossing into the Host's process on every pane-state change,
 ...
 * owner may never enroll a Host at all.
 *
...
import type { RemoteHostConsoleStatus, HostStatusEvent } from '../../host/remote/service-protocol';
import type { RemoteHostLink } from '../../lib/platform/types';

/**
 * Run `arm` while the Host service is enrolled and its disarm when it is not,
 ...
 */
export function armWhileEnrolled(link: RemoteHostLink, arm: () => () => void): () => void {
  ...
  const unsubscribe = link.on('status', (data) => {
    apply(!!(data as HostStatusEvent | null)?.enrolled);
  });
  void link
    .command('status')
    .then((status) => apply(!!(status as RemoteHostConsoleStatus | null)?.enrolled))
    // No Host to report one: nothing to arm, which is already the state.
    .catch(() => {});
