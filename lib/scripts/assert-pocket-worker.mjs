 ...
 *
 * A root-relative reference is the whole test: the bundle mounts at the origin
 * root (Deployment), so anything not starting with `/` is either inline or
...
export function assertPocketShell(outDir) {
  ...
  for (const [, attributes, body] of html.matchAll(SCRIPT_TAG)) {
    ...
    if (!src || !src.startsWith('/')) {
      throw new Error(
        `${SHELL_FILE} has a script that is neither inline nor root-relative (<script${attributes}>). ` +
