...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  try {
    ...
    const diagnosticScripts = assertPocketShell(join(outDir, 'diagnostics'));
    if (diagnosticScripts === 0) throw new Error('the diagnostic shell has no executable entry');
    console.log(`diagnostic shell ok: ${diagnosticScripts} script(s), all same-origin and external`);
  } catch (error) {
