...
// A host binding names real directories and executables: `dor playwright` checks both.
const root = mkdtempSync(join(tmpdir(), 'dor-pw-'));
const firstProject = join(root, 'first-project');
const guiProject = join(root, 'gui');
// The directory `dor playwright` runs from, which the host pins for a key's first command.
...
test('full playwright command forwards native arguments and binds only to playwright', async () => {
  const { calls, options } = fixture();
  const result = await runCli(['playwright', '--key', 'app', 'open', ':5173', '--headed'], options);
...
test('a pinned executable outside the allowlist runs the caller\'s own instead', async () => {
  ...
  for (const binaryPath of ['/bin/sh', './playwright-cli', '/opt/../bin/playwright-cli']) {
    ...
    await runCli(['playwright', 'snapshot'], options);
...
test('raw -s bypasses workspace addressing', async () => {
  const { calls, options } = fixture();
  await runCli(['playwright', '-s=raw-session', 'goto', ':8080'], options);
...
test('surface addressing resolves GUI-created sessions and rejects missing sessions', async () => {
  ...
  await runCli(['playwright', '--surface', 'surface:3', 'snapshot'], options);
  assert.equal(calls[0][1].surface, 'surface:3');
  const missing = fixture();
  assert.equal((await runCli(['playwright', '--surface', 'surface:3', 'snapshot'], missing.options)).exitCode, 1);
  assert.equal(missing.calls.some(c => c[0] === 'exec'), false);
});
test('native failures and close do not create or resurrect panes', async () => {
  const { calls, options } = fixture();
  await runCli(['playwright', 'close'], options);
  assert.equal(calls.some(c => c[0] === 'surface'), false);
  options.execPlaywright = async () => ({ exitCode: 7, stdout: '', stderr: 'native error' });
  assert.deepEqual(await runCli(['playwright', 'open', ':8080'], options), { exitCode: 7, stdout: '', stderr: 'native error' });
});
...
test('identity flags are mutually exclusive', async () => {
  const { calls, options } = fixture();
  assert.equal((await runCli(['playwright', '--key', 'app', '-s', 'raw', 'snapshot'], options)).exitCode, 1);
