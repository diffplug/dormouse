...
- **`SELF_HOST.md`** (repo root) — Self-host deployment: the assistant-run install runbook plus the Installer contract that `SECURITY.md`'s `FAIL IF` lines and `scripts/deploy-lint.mjs` audit.
...
When code covered by a spec changes, change the spec. Where two specs overlap (pane header elements), layout.md owns placement and sizing, alert.md behavior and visual states.

**Say it once.** An invariant and its rationale live in exactly one place — the owning spec, otherwise a comment at the code it constrains; everywhere else gets at most a one-line pointer. A pointer stands alone: name the spec and heading, never paraphrase the target; a paragraph that ends in a cross-spec link is one line, and "same as X, plus this delta" is the only restatement form. Never number another spec's list from outside it. Before cutting a restatement to a pointer, confirm the owner's current text states every fact being cut. Within a spec, a fact lives in one of its invariant list, owning section, or matrix, and the others cite it; a constant's value appears once. Write a comment only for a constraint the code cannot show. Prose is proportional to the change: a one-line fix earns a sentence of spec plus pointers, not three restatements. A "must stay in sync" claim names the test that pins it (exemplars: `lib/src/lib/themes/consumed-keys.test.ts`, `lib/src/lib/mirrored-constants.test.ts`).

**The rationale split.** The spec stays dense and normative; the evidence behind its rules moves to a paired `docs/specs/<foo>.rationale.md`. Opt-in per spec — thin specs keep rationale inline.

- **Maximize the rationale file, minimize the spec.** A new sentence lands in `<foo>.rationale.md` unless it passes the boundary test. A change that grows a spec adds one bolded imperative, with the paragraph behind it in the rationale; a spec budget rising while its rationale's stays flat means the split went backwards.
- **Boundary test.** If deleting a sentence would let a competent editor reintroduce a bug or break compatibility, its one-line form stays in the spec; everything past that line is rationale. Mechanism stays inline only when the invariant is illegible without it. Facts are not evidence: constants, keys, URLs, bounds, direction words, and modal qualifiers (only / never / always) stay in the spec, and two distinct bounds stay distinct. Words that mark evidence: "deliberately", "note that", "is what lets", a parenthetical inside a rule, a tuning note, a reader inventory.
- **Forensic history** — measurement narratives, dead-approach stories, grievances against replaced dependencies — moves to the rationale or is deleted once depreciated; git keeps the record.
...
`scripts/spec-lint.mjs` (`pnpm lint:specs`, the first step of the root `pnpm test`) enforces the mechanically checkable conventions above — its header comment lists the checks — and ratchets size: every spec, rationale file, and this file carries a word budget in `scripts/spec-word-budgets.json`, and growth past it fails — cut, or raise the budget deliberately in the same PR. `SELF_HOST.md`, the one spec outside `docs/specs/`, rides the same checks. Advisory prose reviews follow `docs/prose-audit.md` (`pnpm audit:prose`).
