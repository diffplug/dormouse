...
- **Boundary test.** If deleting a sentence would let a competent editor reintroduce a bug or break compatibility, its one-line form stays in the spec; everything past that line is rationale. Mechanism stays inline only when the invariant is illegible without it.
...
- **Keying.** Each `## X` in `<foo>.rationale.md` is a heading in `<foo>.md`. A rule whose evidence moved gets a trailing `(rationale)` marker — a hint, not a link. Rationale files are informative, not normative: no `## Future`, no `Reserved:`, no `Source of truth:` obligations; date measurements (`measured in Safari 26.5, 2026-08`) so later pruning is safe.
