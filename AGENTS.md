...
- **`SELF_HOST.md`** (repo root) — Self-host deployment: the assistant-run install runbook plus the Installer contract that `docs/specs/security-application.md`'s `FAIL IF` lines and `scripts/deploy-lint.mjs` audit.
...
- **`docs/specs/security-application.md`** — The audited checks on the product's own boundaries: remote control (trust boundary, relay allowlist, credentials at rest, network posture, what crosses) and loopback listeners.
...
| `scripts/loopback-lint.mjs` (`pnpm lint:loopback`) | `docs/specs/security-application.md` -> "Loopback Listeners": a loopback bind is not an access control — a new listener references a guard module or is allowlisted with a reason. |
| `scripts/deploy-lint.mjs` (`pnpm lint:deploy`) | `docs/specs/security-application.md` -> "Credentials at rest" and "Network posture (self-hosted)": the installer controls binding all three of `deploy/local/install-{macos,windows,linux}`. |
| `scripts/e2e-lint.mjs` (`pnpm lint:e2e`) | The structural half of `docs/specs/security-application.md` -> "Remote Control": one Noise suite with no selector, no JavaScript curve, no legacy relay discriminant, no Server-side protocol-v1 type, no checked-in service worker, no optional field on a ciphertext or transcript. |
