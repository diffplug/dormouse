...
- **`docs/specs/tiling-engine.md`** — **Lath**, the in-house headless tiling engine replacing dockview-react. Implemented and inert: the pure core — split-tree model + `layout()`, the op set with Door restore tokens, sash/neighbor/autoEdge geometry. Still design (`## Future`): hierarchical pointer DnD with depth cycling, the native FLIP animation contract, the never-re-parent DOM binding, the dockview deletion inventory, and the staged **lath-rollout** scope. layout.md remains the source of truth for shipped layout behavior until promotion. Touch points: `lib/src/lib/lath/` (nothing imports it yet).
