...
- **`docs/specs/website-docs.md`** — Public documentation on the marketing site: the two generated references (`/docs/dor`, `/docs/agent-skill`), the Markdown rendering contract they share, `vscode-ext/README.md` as the canonical product guide published off-site, and the validation lint that pins their links.
...
Five sibling lints run alongside it in `pnpm test`, each enforcing one invariant a spec states in prose: `scripts/public-docs-lint.mjs` (`pnpm lint:public-docs`) for the public-doc contracts in `docs/specs/website-docs.md`; `scripts/xterm-lint.mjs` (`pnpm lint:xterm`) for the `@xterm/*` version lockstep in `docs/specs/webgl-text.md`; `scripts/loopback-lint.mjs` (`pnpm lint:loopback`) for the rule in `SECURITY.md` -> "Loopback Listeners" that a loopback bind is not an access control — a new listener must reference a guard module or be allowlisted with a reason; `scripts/deploy-lint.mjs` (`pnpm lint:deploy`) for the installer controls in `SECURITY.md` -> "Credentials at rest" and "Network posture (self-hosted)", which bind all three of `deploy/local/install-{macos,windows,linux}`; and `scripts/e2e-lint.mjs` (`pnpm lint:e2e`) for the structural half of `SECURITY.md` -> "Remote Control" — one Noise suite with no selector, no JavaScript curve, no legacy relay discriminant, no Server-side protocol-v1 type, no checked-in service worker, and no optional field on a ciphertext or transcript. Each rule names the `SECURITY.md` line it enforces and fails if that line is gone.
