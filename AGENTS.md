...
- **`docs/specs/tiling-engine.md`** — Design-stage spec (nothing implemented): **Lath**, the planned in-house headless tiling engine replacing dockview-react — pure split-tree model + `layout()`, the op set with Door restore tokens, hierarchical pointer DnD with depth cycling, the native FLIP animation contract, the never-re-parent DOM binding, the dockview deletion inventory, and the staged **lath-rollout** scope. layout.md remains the source of truth for shipped layout behavior until promotion. Touch points: none yet — design only.
