...
- **Default to the rationale.** A change adds one bolded rule to the spec and the paragraph behind it to `<foo>.rationale.md`; only the boundary test below keeps a sentence in the spec. A spec growing while its rationale does not is the split running backwards.
