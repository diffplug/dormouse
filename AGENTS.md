...
**The rationale split.** The spec stays dense and normative; the evidence behind its rules moves to a paired `docs/specs/<foo>.rationale.md`. Opt-in below 2,500 words — thin specs keep rationale inline; past that the lint requires the file.

- **Maximize the rationale file, minimize the spec.** A new sentence lands in `<foo>.rationale.md` unless it passes the boundary test. A change that grows a spec adds one bolded imperative, with the paragraph behind it in the rationale; a spec budget rising while its rationale file stays flat means the split went backwards.
...
`scripts/spec-lint.mjs` (`pnpm lint:specs`, the first step of the root `pnpm test`) enforces the mechanically checkable conventions above — its header comment lists the checks — and ratchets size: every spec and this file carry a word budget in `scripts/spec-word-budgets.json`, set at the file's size plus 32 words rounded up to the nearest 10 — room for most of one rule. Rationale files carry none; evidence may grow without limit. When a spec exceeds its budget, cut to fit; if you cannot, add what is needed and re-baseline with `node scripts/spec-lint.mjs --ratchet <spec>` in the same PR. `SELF_HOST.md`, the one spec outside `docs/specs/`, rides the same checks. Advisory prose reviews follow `docs/prose-audit.md` (`pnpm audit:prose`).
