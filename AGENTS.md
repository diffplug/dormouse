...
- **`docs/specs/tiling-engine.md`** — **Lath**, the in-house headless tiling engine replacing dockview-react. Implemented: the pure core (split-tree model + `layout()`, the op set with Door restore tokens, sash/neighbor/autoEdge geometry, the headless animator, hierarchical hit-testing) and, behind the `dormouse.flags.lath` dev flag, the full Wall binding — the never-re-parent LathHost with native motion and depth-cycling pointer DnD, the wall store + engine, the pane props contract, and dual-write persistence with legacy migration. Still design (`## Future`): stage 5, the dockview deletion sweep closing the staged **lath-rollout** scope. Flag off, layout.md remains the source of truth for the shipped dockview path. Touch points: `lib/src/lib/lath/`, the `lath-*` modules + `LathHost.tsx` in `lib/src/components/wall/`, the lath branches in `Wall.tsx`.
