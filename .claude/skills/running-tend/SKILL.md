...
## Nightly's tend workflow regeneration proposes a *downgrade* of the uv pins — don't ship it

`.github/renovate.json` enables the `github-actions` manager, so Renovate bumps the `astral-sh/setup-uv` action pin and its `version:` input inside the generated `.github/workflows/tend-*.yaml` files ([#636](https://github.com/diffplug/dormouse/pull/636) took them to `v10.1.0` / uv `0.12.13`). The tend generator emits whatever its own release pins, which trails Renovate — at tend 0.2.7 that is `v10.0.1` / uv `0.12.10`.

So `nightly_workflow_update.py prepare` reports `changed: true` on a same-version regeneration whose entire diff is that downgrade, in `tend-mention.yaml` and `tend-notifications.yaml`. **Never ship it.** Restore the repo's higher pins in the prepared worktree, confirm the staged diff is then empty, remove the worktree and the `$TMPDIR/tend-workflow-update.json` state by hand (the script has no abort command), and skip the PR. Shipping it would revert a Renovate bump that Renovate re-applies on its Monday schedule, so the two would trade the pin back and forth every week.
