#!/usr/bin/env node
/**
 * Mechanical check for the self-host installer invariants in `docs/specs/security-application.md`
...
export const RULES = [
  ...
  },
  {
    ...
    skip: {
      macOS:
        'the per-file walk is Windows-only — `server/src/state.ts` writes 0o600 and it holds on unix, so the mode check on `state/` is the whole control (docs/specs/security-application.md -> "Credentials at rest")',
      Linux:
  ...
  {
    // The same ordering control on `config/server.env`, which docs/specs/security-application.md's
  ...
  {
    // `run/` is the whole claim — docs/specs/security-application.md's FAIL IF carries the why — so
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (failures.length > 0) {
    console.error('deploy-lint: the installers no longer hold controls docs/specs/security-application.md requires\n');
    for (const failure of failures) console.error(`  ${failure}\n`);
    console.error(
      'Each line above maps to a FAIL IF in docs/specs/security-application.md. If a control moved rather than\n' +
