...
mac_app_path() {
    require_directory \
        "macOS app bundle" \
        "$SIGN_DIR/standalone-mac-aarch64/src-tauri/target/aarch64-apple-darwin/release/bundle/macos/MouseTerm.app"
}
...
windows_exe_path() {
    require_file \
        ...
        "$(windows_release_dir)/mouseterm.exe"
}

windows_installer_path() {
    local version="${1:-}"
    local pattern="MouseTerm_*_x64-setup.exe"
    if [[ -n "$version" ]]; then
        pattern="MouseTerm_${version}_x64-setup.exe"
    fi
...
linux_appimage_path() {
    ...
    require_single_find_match \
        ...
        -type f \
        -name "MouseTerm_${version}_amd64.AppImage"
}
