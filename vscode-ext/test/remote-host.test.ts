 ...
 * (`docs/specs/security-application.md` -> "Trust boundary"), so fixtures have to be real records. The
...
describe('remote host service glue', () => {
  ...
  it('forwards a command to the window that holds the Host', async () => {
    ...
    // A service→webview shape: the one-time token is not in it (docs/specs/security-application.md -> "Trust boundary").
