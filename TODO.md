...
- [ ] 3. Pin GitHub Actions to full commit SHAs and use automation to keep pins updated.
- [ ] 4. Add explicit workflow permissions, with read-only defaults and no `id-token: write` unless a job truly needs it.
- [ ] 5. Put VS Code Marketplace and OpenVSX publishing behind a protected GitHub environment with reviewer approval and tag restrictions.
- [ ] 6. Verify CI artifacts before local signing, ideally with GitHub artifact attestations checked by `scripts/sign-and-deploy.sh`.
