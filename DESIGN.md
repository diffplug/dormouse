...
**The Viewport-Bound Rule.** Anything floating over the viewport takes its height cap from `OVERLAY_MAX_HEIGHT` in `design.tsx` — `.modal` for a `ModalFrame` surface (the viewport minus `MODAL_OVERLAY_INSET` doubled), `.popover` for an anchored overlay (matching `clampOverlayPosition`'s margin). Don't hand-write a `vh`/`dvh` literal at the call site: the six that predated this token had drifted to five different budgets, and one silently shadowed its own overlay's padding. Both entries read `--overlay-max-h` first, so a story — or a host with less room than the window — can narrow the bound without touching the component. A deliberately *smaller* budget than the viewport (a context menu at `max-h-[70vh]`) is a different decision and stays at the call site.
...
- **Don't** call `window.alert` / `confirm` / `prompt`. The standalone webview implements no dialog panels, so `confirm` resolves `false` and `alert` shows nothing — a control gated on one silently does nothing on the desktop app (this is how the theme uninstall shipped broken). Use `ModalFrame`, or make the action a single click when it is cheap and reversible.
